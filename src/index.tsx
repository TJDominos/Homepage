...
function App() {
  ...
  const renderActiveTabContent = (isDesktopView: boolean) => {
    ...
    if (activeTab === "inbox") {
      return (
        <div className="text-center py-[80rem] flex-1">
          <h2 className="text-[24rem] font-bold text-black mb-[8rem]">Inbox Page</h2>
      ...
      {(isDesktop || activeTab !== "money") && (
        <div className="home-header">
          <div className="home-header-content">
            ...
            <div className="flex items-center gap-[12rem] shrink-0">
              {userAccount ? (
                <button
                  className="account-toggle-button w-[32rem] h-[32rem] rounded-full overflow-hidden border border-black/10 bg-[#f0f2f5] text-slate-500 transition-colors shadow-sm relative z-[101]"
                  onClick={() => isAccountModalOpen ? setAccountModalOpen(false) : setAccountModalOpen(true)}
                >
                  <img
                    src={getSysAvatar("01")}
                    alt="User Avatar"
                    ...
                    onError={(e) => {
                      ...
                      (e.target as HTMLImageElement).parentElement!.innerHTML = '<div class="w-full h-full flex items-center justify-center bg-[#f0f2f5] text-slate-500"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg></div>';
                    }}
                  />
                </button>
              ) : (
          ...
          {isDesktop && (
            <div className="border-t border-black/5 hidden md:flex items-center h-[52rem]">
              <div className="flex items-center gap-[12rem] w-full max-w-[var(--max-content-width-pc)] mx-auto px-[16rem]">
                {[
                  ...
                  <button
                    ...
                    className={`px-[20rem] h-[36rem] rounded-[10rem] font-bold text-[14rem] transition-colors capitalize flex items-center ${
                      activeTab === tab.id 
                        ? "bg-white text-black shadow-[0_1rem_4rem_rgba(0,0,0,0.06)] border border-black/5" 
                        : "text-slate-500 hover:text-black hover:bg-black/5"
                    }`}
                  >
                    <div className="mr-[8rem] w-[18rem] h-[18rem] flex items-center justify-center">
                       {tab.id === 'money' && <svg className="w-full h-full" viewBox="0 0 27 27" fill="none"><path d={svgPaths.p1bf39e00} fill="currentColor" /></svg>}
                       {tab.id === 'play' && (
                         <svg className="w-full h-full" viewBox="0 0 30 30" fill="none">
                           <circle cx="15" cy="15" fill={activeTab === 'play' ? "currentColor" : "none"} r="15" />
                           {activeTab !== 'play' && <circle cx="15" cy="15" stroke="currentColor" strokeWidth="1" fill="none" r="14.5" />}
                           <g>
                             <path d={svgPaths.p170a5300} stroke={activeTab === 'play' ? "#fff" : "currentColor"} strokeWidth="1.4728" />
                             <path d={svgPaths.p12adf040} stroke={activeTab === 'play' ? "#fff" : "currentColor"} strokeWidth="1.26015" />
                             <circle cx="20.8592" cy="20.1715" r="2.27122" stroke={activeTab === 'play' ? "#fff" : "currentColor"} strokeWidth="1.18139" />
                           </g>
                         </svg>
                       )}
                       {tab.id === 'rank' && <Trophy size={18} strokeWidth={activeTab === 'rank' ? 2 : 1.5} />}
                       {tab.id === 'history' && <svg className="w-full h-full" viewBox="0 0 27 27" fill="none"><path d={svgPaths.p2891d1d0} fill="currentColor" /></svg>}
                    </div>
                    {tab.label}
                  </button>
                ))}
