...
export function RegistrationModal({ isOpen, onClose, onSubmit }: RegistrationModalProps) {
  ...
  const [verifyStatus, setVerifyStatus] = useState<'idle' | 'error' | 'success'>('idle');
    ...
    if (isOpen) {
      ...
      try {
        ...
        if (saved) {
          ...
          setSelectedAvatar(parsed.avatarUrl || (parsed.avatarCode !== 'custom' ? getSysAvatar(parsed.avatarCode || "") : "") || "");
          setEmail(parsed.email || "");
          setSubscribe(parsed.subscribe !== undefined ? parsed.subscribe : true);
          setBio(parsed.bio || "");
          setLocation(parsed.location || "United Kingdom");
          setNotifications(parsed.notifications !== undefined ? parsed.notifications : false);
        } else {
          setName("");
          setSelectedAvatar("");
          setEmail("");
  ...
  const saveProfileData = () => {
    try {
      const isCustom = selectedAvatar.startsWith('data:') || selectedAvatar.startsWith('http') || selectedAvatar.startsWith('/');
      const profileData = {
        username: name || "User",
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div 
          className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-auto bg-black/60 backdrop-blur-sm px-4"
        >
          <motion.div
            ...
            {step === "PROFILE" ? (
              <div className="flex flex-col h-full overflow-y-auto hidden-scrollbar">
                <div className="flex flex-row items-center justify-start px-4 pt-4 pb-4 sticky top-0 bg-[#dce0ef] z-10">
                  <button onClick={handleCloseAttempt} className="hover:opacity-80 transition-opacity flex items-center font-medium pr-4">
                    <ChevronLeft strokeWidth={2.5} size={20} className="mr-4 text-black" />
                    <span className="text-[14px] font-medium text-black">Profile Setting</span>
                  </button>
                </div>
                ...
                <div className="px-4 pb-4">
                  <div className="bg-[#e4e5f0] border border-white/40 rounded-2xl p-4 shadow-sm font-sans flex flex-col gap-4">
                    ...
                    {/* Avatar Selection */}
                    <div>
                      <div className="text-[14px] text-black mb-1.5 font-medium tracking-tight">Avatar</div>
                      <div className="flex items-center gap-4">
                        <div className="w-[70px] h-[70px] rounded-full border-[1.5px] border-black flex items-center justify-center overflow-hidden bg-white shrink-0">
                          {selectedAvatar ? (
                            <img 
                              src={selectedAvatar.startsWith('data:') || selectedAvatar.startsWith('http') || selectedAvatar.startsWith('/') ? selectedAvatar : getSysAvatar(selectedAvatar)} 
                              alt="selected" 
                    ...
                    {/* Name */}
                    <div>
                      <div className="text-[14px] font-medium mb-2.5 text-black tracking-tight"><span className="text-red-500 mr-1">*</span>Name</div>
                    ...
                    {/* Email */}
                    <div>
                      <div className="text-[14px] font-medium mb-2.5 text-black tracking-tight"><span className="text-red-500 mr-1">*</span>Email</div>
                      <input
                        type="email"
                        value={email}
                    ...
                    <label className="flex items-start gap-4 cursor-pointer group mt-0">
                      <div className={`w-5 h-5 rounded-md flex items-center justify-center shrink-0 transition-colors mt-1 ${subscribe ? 'bg-[#7E57C2]' : 'border-[2px] border-black/30'}`}>
                        {subscribe && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>}
                      </div>
                      <span className="text-[14px] text-black tracking-tight leading-snug">Subscribe for special offers and promotions.</span>
                      <input type="checkbox" className="hidden" checked={subscribe} onChange={(e) => setSubscribe(e.target.checked)} />
                    </label>

                    {/* Bio */}
                    <div>
                      <div className="text-[14px] font-medium mb-2.5 text-black tracking-tight">Bio</div>
                      <div className="relative">
                        <textarea
                          value={bio}
                          onChange={(e) => {
                        ...
                        <span className="absolute right-4 bottom-3 text-[14px] text-slate-400 pointer-events-none tracking-tight">
                          {bio.trim() === '' ? 0 : bio.trim().split(/\s+/).length}/50
                        </span>
                      </div>
                    </div>
                    ...
                    {/* Location */}
                    <div>
                      <div className="text-[14px] font-medium mb-2.5 text-black tracking-tight"><span className="text-red-500 mr-1">*</span>Location</div>
                      <div className="relative">
                        <select 
                          value={location}
                    ...
                    <div className="flex items-center justify-between pt-4">
                      <div className="text-[14px] font-medium text-black tracking-tight">Enable notifications</div>
                      <button 
                      ...
                      >
                        <div className={`absolute left-[2px] w-[20px] h-[20px] bg-white rounded-full shadow-sm transition-transform ${notifications ? 'translate-x-[20px]' : 'translate-x-0'}`}>
                        </div>
                      </button>
                    </div>
                    ...
                    <div className="space-y-4 pt-4">
                      <a href="#" className="text-[14px] text-[#7E57C2] underline underline-offset-2 hover:opacity-80 block tracking-tight text-center transition-opacity">Randseed Privacy Policy</a>
                      <button 
              ...
              <div className="flex flex-col h-full">
                <div className="flex items-center justify-between px-4 pt-4 pb-4">
                  ...
                  <button onClick={onClose} className="text-[#8468d6] hover:text-[#6a48a5] transition-colors text-[14px] pr-4 font-medium">
                    Skip
                  </button>
                </div>

                <div className="px-4 pb-4 flex-1">
                  <h2 className="text-[14px] font-bold text-black mb-4">Send verification code to Email</h2>
                    ...
                    <div>
                      ...
                      <div className="h-[20px]">
                        {verifyStatus === 'error' && verifyCode === '112344' && (
                          <p className="text-red-500 text-[11px] mt-1 ml-3">Wrong verification code</p>
                        )}
                        {verifyCode === '112345' && (
                          <p className="text-red-500 text-[11px] mt-1 ml-3">Limit xx times, please try again after xx minutes</p>
                        )}
                      </div>
                    </div>
              ...
              <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/60 backdrop-blur-sm pointer-events-auto px-4">
                ...
                >
                  ...
                  <div className="text-center font-sans pt-4">
                    <h3 className="text-[14px] font-medium text-black tracking-tight mb-4">Exit the page</h3>
                    <p className="text-[14px] leading-[1.4] text-slate-600 mb-4">If you exit the page, the filled in content will be lost</p>
