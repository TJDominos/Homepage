    ...
    <div className="absolute inset-0 flex items-center justify-center gap-[4rem]">
      {nums.map((num, i) => {
        ...
        return (
          <motion.div
            key={i}
            className={`w-[16rem] h-[16rem] rounded-full shadow-md flex items-center justify-center font-bold text-white border border-white/60 text-[10px] relative ${isLast ? 'bg-[#ff4d4d]' : 'bg-[#18181b]'}`}
            animate={{ 
              y: [0, -5, 0],
            }}
            transition={{ 
