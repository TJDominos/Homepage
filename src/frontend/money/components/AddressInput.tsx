  ...
  const handleSaveConfirm = () => {
    if (editingId) {
      setSavedAddresses((prev) =>
        prev.map((a) => (a.id === editingId ? { ...a, name: addressName } : a)),
      );
    } else {
      setSavedAddresses((prev) => [
        { id: Date.now().toString(), name: addressName, address: value },
        ...prev,
      ]);
    }
          ...
          {savedAddresses.map((item) => (
            ...
            >
              <div className="flex flex-col min-w-0 pr-4">
                {item.name && (
                  <span className="text-[12px] text-[#6A3FE6] font-medium mb-0.5">
                    {item.name}
                  </span>
                )}
      ...
      {showNameModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <div className="relative w-full max-w-[320px] bg-[#EAEAEA] rounded-3xl p-6 flex flex-col pointer-events-auto shadow-xl">
            <h3 className="text-[18px] font-bold text-black text-center mb-6">
              Address Name
            </h3>
            <div className="relative mb-6">
              <input
                ...
                value={addressName}
                onChange={(e) =>
                  setAddressName(e.target.value.substring(0, 20))
                }
