...
import { WltLogo } from "../../../components/layout/WltLogo";
...
export function DepositTab({ isDesktop }: DepositTabProps) {
  ...
  useEffect(() => {
    if (crypto !== "USDC" && crypto !== "USDT" && network !== "SOL") {
      setNetwork("SOL");
    }
  }, [crypto, network]);
    ...
    <div className={`mt-6 gap-2 flex flex-col`}>
      <div className="flex flex-col gap-1.5">
        ...
        <p className="text-[12px] font-normal text-black/65">
          Only USDC and USDT deposits are convertible to Gcoin. All assets are convertible to Bonus.
        </p>
      </div>
      <div
